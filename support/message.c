...
message_init(FILE* logFP)
{
  ...
  log_d("message_init: ready at port '%d'\r", port);
...
{
  if (ourSocket == 0) {
    log_v("message_send: called before message_init\r");
    return; // error in usage of this function.
  }
  if (message == NULL) {
    log_v("message_send: called with null message\r");
    return; // error in usage of this function.
  }
  if (sendto(ourSocket, message, strlen(message), 0,
    ...
    log_s("message_send: TO %s\r", message_stringAddr(to));
    log_d("message_send: %d lines:\r", numLines(message));
    log_s("%s\r", message);
  }
}
  ...
  while (true) {
	...
	return false; // error
      ...
      if (FD_ISSET(ourSocket, &rfds)) {
        ...
        log_v("message_loop: message ready on socket\r");
	    ...
	    // record it
	    log_s("message_loop: FROM %s\r", message_stringAddr(sender));
	    log_d("message_loop: %d lines:\r", numLines(buf));
	    log_s("%s\r", buf);
